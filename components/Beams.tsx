...
function extendMaterial(BaseMaterial, cfg) {
  ...
  for (const [includeToken, code] of Object.entries(cfg.vertex ?? {})) {
    vert = vert.replace(includeToken, `${includeToken}\n${code}`)
  }
  for (const [includeToken, code] of Object.entries(cfg.fragment ?? {})) {
    frag = frag.replace(includeToken, `${includeToken}\n${code}`)
  }
...
function createStackedPlanesBufferGeometry(n: number, width: number, height: number, spacing: number, heightSegments: number) {
  ...
  for (let index = 0; index < n; index += 1) {
    ...
    for (let segment = 0; segment <= heightSegments; segment += 1) {
      ...
      const firstVertex = [xOffset, y, 0]
      const secondVertex = [xOffset + width, y, 0]
      positions.set([...firstVertex, ...secondVertex], vertexOffset * 3)
...
const MergedPlanes = forwardRef(function MergedPlanes(
  { material, width, count, height }: { material: THREE.Material; width: number; count: number; height: number },
  ...
  useFrame((_, delta) => {
    const currentMesh = mesh.current as THREE.Mesh & { material: { uniforms?: { time?: { value: number } } } }
    if (currentMesh?.material?.uniforms?.time) {
      currentMesh.material.uniforms.time.value += 0.1 * delta
    }
  })
...
const PlaneNoise = forwardRef(function PlaneNoise(
  props: { material: THREE.Material; width: number; count: number; height: number },
  ref: React.ForwardedRef<THREE.Mesh>,
) {
...
function DirLight({ position, color }: { position: [number, number, number]; color: string }) {
  ...
  useEffect(() => {
    ...
    const camera = dir.current.shadow.camera
    if (!camera) return
    camera.top = 24
    camera.bottom = -24
    camera.left = -24
